        ...
        <div class="col-md-8">
            <div class="card">
                <div class="d-flex justify-content-left mb-2">
                    <a href="{{ route('demande') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Retour</a>
                </div><br>
                <div class="card-header">Détails de la demande</div>

                <div class="card-body">
                    <p><strong>Émetteur :</strong> {{ $demande->emetteur->name }}</p>
                    <p><strong>Message :</strong> {{ $demande->message }}</p>

                    @if ($demande->demande_statut_id === 'en_cours')
                    <div class="alert alert-warning">En cours</div>
                    <div class="mb-3">
                        <form method="POST" action="{{ route('demandes.update', ['id' => $demande->id]) }}">
                            @csrf
                            @method('PUT')
